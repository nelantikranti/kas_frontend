    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        ...
        <div className="bg-white p-4 sm:p-6 rounded-lg shadow border border-gray-200">
          ...
          <div className="text-2xl sm:text-3xl font-bold text-green-600">{stats.contacts.contacted || stats.contacts.replied || 0}</div>
        </div>
        <div className="bg-white p-4 sm:p-6 rounded-lg shadow border border-gray-200">
        ...
        <div className={`mb-4 text-sm rounded-lg px-4 py-2 inline-block ${
          ...
          {activeTab === "contacts" ? (
            filteredContacts.length > 0 ? (
              <>Showing <span className="font-semibold text-green-700">{filteredContacts.length}</span> of <span className="font-semibold">{contacts.length}</span> contacts</>
            ) : (
              <>No contacts found for "<span className="font-semibold">{searchQuery}</span>"</>
            )
          ) : (
            filteredDemos.length > 0 ? (
              <>Showing <span className="font-semibold text-green-700">{filteredDemos.length}</span> of <span className="font-semibold">{demos.length}</span> demo requests</>
            ) : (
