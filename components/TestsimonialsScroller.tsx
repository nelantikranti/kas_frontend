// Filename intentionally 'TestsimonialsScroller' to avoid accidental duplicates; export default below.
"use client";
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        // If less than 3, duplicate until at least 3 for smooth scroll
        let normalized = list.slice();
        while (normalized.length > 0 && normalized.length < 3) {
          normalized = normalized.concat(list);
        }

        // Duplicate once more for seamless infinite scroll
        const doubled = normalized.concat(normalized);
        setItems(doubled);
      } catch (err) {
            ...
            >
              ...
              >
                {items.map((t: any, idx: number) => (
                  <article
                    key={`${t.id || idx}-${idx}`}
                    className="min-w-[260px] sm:min-w-[320px] bg-white rounded-[18px] border border-gray-100 shadow-lg p-5 flex flex-col hover:-translate-y-1 hover:shadow-2xl transition-transform duration-300"
                    role="group"
                  >
                    ...
                    <div className="mt-4 text-gray-400 text-5xl opacity-10 absolute right-6 top-6 pointer-events-none">
                      <IoChatbubbles />
                    </div>
                  </article>
                ))}
